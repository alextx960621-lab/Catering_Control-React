...
export default function UsersPage({ user }) {
  ...
  const columns = [
    ...
    { key: 'id', label: 'Acciones', render: (u) => (
      <><button className="icon-btn info" onClick={() => setEditing(u)}>Editar</button>{u.id !== user.id && <button className="icon-btn delete" onClick={() => handleDelete(u)}>Quitar</button>}</>
    ) },
  ];
  ...
  const roleColumns = [
    ...
    { key: 'id', label: 'Acciones', render: (r) => (
      <><button className="icon-btn info" onClick={() => setEditingRole(r)}>Editar</button><button className="icon-btn delete" onClick={() => handleRoleDelete(r)}>Quitar</button></>
    ) },
  ];
  ...
  return (
    <section className="page active">
      ...
      <div className="page-head" style={{ marginTop: 26 }}>
        ...
        <div className="head-actions"><button className="info" onClick={() => setEditingRole({})}>+ Crear rol</button></div>
      </div>
