...
### Sigue abierto
1. **Confirmar que `resolve-maps-link` esté desplegada** (y con la versión más reciente, la que manda `User-Agent` y responde siempre 200) — el `.rar` de esta ronda ya trae `supabase/.temp/` (señal de que se corrió `supabase link` en algún momento), pero no hay forma de confirmar desde acá si el deploy en sí se llegó a correr. Si hace falta:
   ```
   supabase functions deploy resolve-maps-link --no-verify-jwt
   ```
   Para confirmar que quedó bien: pegar un link corto de Maps (`maps.app.goo.gl/...`) en una dirección de Clientes, salir del campo, y ver que aparece "✓ resuelto" al lado de "Coordenadas".
