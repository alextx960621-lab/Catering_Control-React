...
-- --------------------------------------------------------------------------
-- 20. Notificaciones push (recordatorio de plan). Requiere las extensiones
--     pg_cron y pg_net -- si el "create extension" de acá abajo da error de
--     permisos, andá a Dashboard → Database → Extensions y activá "pg_cron"
--     y "pg_net" a mano, después seguí corriendo el resto de esta sección.
--
--     save_push_subscription / remove_push_subscription: las llama el
--     propio cliente desde el portal (con su token de sesión, igual que el
--     resto de sus RPCs). get_clients_for_push_reminder(): usada SOLO por
--     la Edge Function `send-push` (con la service_role key) para el cron
--     diario -- no es alcanzable con la anon key.
--
--     "Últimos 3 días del plan": remaining = paidDays - consumedDays, el
--     mismo cálculo que usa el portal del cliente (Portal.jsx). Solo
--     clientes 'Activo' (no tiene sentido avisarle a uno pausado).
--
--     El cron de más abajo asume América/La Paz (UTC-4 todo el año, sin
--     horario de verano) -- 09:00 La Paz = 13:00 UTC. Si esta empresa
--     opera en otro huso horario, cambiá el "13". Reemplazá también
--     <CRON_SECRET> por el mismo valor que configures como secreto
--     CRON_SECRET de la Edge Function `send-push` (supabase/functions/
--     send-push/index.ts) ANTES de correr el bloque de cron.schedule.
-- --------------------------------------------------------------------------
...
--
--   select * from cron.job where jobname = 'push-recordatorio-plan-diario';
--   -- confirma que el cron de notificaciones de la sección 20 quedó
--   -- programado (después de reemplazar <CRON_SECRET> ahí arriba).
--   select * from get_clients_for_push_reminder(3);
--   -- lista quién recibiría el recordatorio automático HOY si corriera ya.
--
